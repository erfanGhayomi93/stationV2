...
const OrderInfoModal = ({ modalData, setModalData, aggregateType }: Props) => {
                        ...
                        value.reduce(
                            (a: typeof footerData, b: Record<string, any>, c: number) => {
                                return {
                                    ...
                                    row: a.row + c + 1,
                                    orderValue: a.orderValue + b.orderValue,
                                };
                            },
                            { comission: 0, price: 0, quantity: 0, row: 0, orderValue: 0 } as typeof footerData,
                        ),
                    );
                }
            },
    ...
    const colDefs = useMemo(
        (): ColDefType<unknown>[] => [
            ...
            { headerName: 'ارزش معامله', field: 'totalValue', maxWidth: 100, type: 'sepratedNumber' },
    ...
    const createTitle = () => {
        switch (aggregateType) {
            case 'Customer':
                return `(مشتری - ${modalData?.data?.customerTitle})`;
            case 'Symbol':
                return `(نماد - ${modalData?.data?.symbolTitle})`;
            default:
                return '';
        }
    };
        ...
        >
            <div className="h-full w-full flex flex-col p-4 justify-between gap-3">
                ...
                <div className="rounded-lg w-full h-14 mb-1 shadow-[0_0_11px_0_rgba(0,0,0,0.06)] border border-L-gray-300 flex items-center p-4 gap-6">
                    ...
                    <Seperator height={'50%'} />
                    <InfoField label="ارزش معامله" value={footerData.orderValue} preFix preFixText="ریال" />
                </div>
            </div>
        </AppModal>
    );
