...
import { useSymbolSearchQuery } from 'src/app/queries/tradingView';
...
const TvSelectSymbolModal = ({ actionId }: TvSelectSymbolModalProps) => {
	...
	const { data: symbolData, refetch, isFetching } = useSymbolSearchQuery(search, {
		enabled: false
	});
							...
							<div className='flex items-center absolute text-L-gray-500 dark:text-D-gray-500 left-3'>
								...
								{(search.length < 2) && (
									<span className='pointer-events-none text-sm'>
										{t('tv_chart.enter_less_than_two_char')}
									</span>
								)}
							</div>
						</label>
