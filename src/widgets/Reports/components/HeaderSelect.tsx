...
const HeaderSelect = ({ displayName, onChange, value }: IProps) => {
    const [isSelectAvtive, setIsSelectAvtive] = useState(false);
    ...
    useEffect(() => {
        isSelectAvtive && inputRef.current?.focus();
    }, [isSelectAvtive]);

    return isSelectAvtive ? (
        <select
            ref={inputRef}
            value={value}
            ...
            onBlur={() => setIsSelectAvtive(false)}
        >
            {options.map((i, x) => (
        ...
        <div className="w-full flex items-center justify-center gap-3" onClick={() => setIsSelectAvtive(true)}>
            <SearchIcon />
