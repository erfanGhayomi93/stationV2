...
const DivideOrderModal = () => {
            ...
            queryClient.setQueryData(['divideOrderCache'], (oldData: DividedOrderRowType[] | undefined) => {
                if (!!oldData) {
                    return oldData.map(item => {
                        if (!!receivedData[item.id]) return { ...item, clientKey: receivedData[item.id] }
                        return { ...item }
                    })
                }
            })
        ...
        queryClient.setQueryData(['divideOrderCache'], (oldData: DividedOrderRowType[] | undefined) => {
            if (!!oldData) {
                return oldData.map(item => (
                    {
                        ...item,
                        status: item.status ? item.status : 'InOMSQueue',
                    }
                ))
            }
        })
    ...
    const sendOneOrder = (orderId: string) => {
        const customer = customers?.find(item => item.id === orderId)
    ...
    onOMSMessageHandlerRef.current = useMemo(
        () => (message: Record<number, string>) => {
            ...
            const updatedCustomers = (status: OrderStatusType, omsClientKey: string, isError: boolean) => {
                ...
                queryClient.setQueryData(['divideOrderCache'], (oldData) => {
                    if (!!oldData) {
                        const customersOld = JSON.parse(JSON.stringify(oldData)) as DividedOrderRowType[];
                        const findItem = customersOld.find(item => item.clientKey === omsClientKey) as DividedOrderRowType
                        const indexItem = customersOld.indexOf(findItem)
                        const newItem = { ...findItem, status: status, isError }

                        if (indexItem >= 0) customersOld[indexItem] = newItem
                        return customersOld
                    }
                })
            ...
            timer = setTimeout(() => {
                if (["Error"].includes(omsOrderStatus)) {
                    updatedCustomers(orderMessageType, omsClientKey, true)
                } else if (!!omsOrderStatus) {
                    updatedCustomers(omsOrderStatus, omsClientKey, false)
                }
                        ...
                        <div className="w-56">
                            <ControllerInput
                                ...
                                onChange={(value) => {
                                    ...
                                    queryClient.setQueryData(['divideOrderCache'], (oldData: DividedOrderRowType[] | undefined) => {
                                        if (!!oldData) {
                                            return oldData.map(item => ({ ...item, price: value, status: undefined, clientKey: undefined }))
                                        }
                                    })
                                }}
