...
export const useSetPlatformSetting = () =>
    useMutation(savePlatformSetting, {
        onSuccess: ({ result }) => {
            if (result) {
...
export const setUsernameFn = async (params: { newUserName: string; userId: number }) => {
    const { data } = await AXIOS.post<GlobalApiResponseType<boolean>>(Apis().Setting.UpdateUserName, params);
    return data;
};

export const useSetUsername = () =>
    useMutation(setUsernameFn, {
        onSuccess: ({ result }) => {
            if (result) {
                queryClient.invalidateQueries([Apis().Setting.GetUserSetting]);
            }
        },
    });
