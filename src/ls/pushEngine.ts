...
const subscribe = <T = IChangedField>({ id, mode, items, fields, adapterName, isSnapShot, onFieldsUpdate }: ISubscribe<T>) => {
     ...
     sub.addListener({
          onItemUpdate: updateInfo => {
               ...
               if (updatedFields.itemName && Object.keys(updatedFields.changedFields).length > 0)
                    onFieldsUpdate(updatedFields as any);
          },
     });
     ...
     console.log(
          `%c SUBSCRIBED ${id}`,
          'background: linear-gradient(90deg, rgba(12,10,133,1) 17%, rgba(0,134,255,1) 100%); color: #ffffff; border-radius: 10px; padding: 5px'
     );
};
...
const unSubscribe = (subId: string) => {
     ...
     if (subscriptions[subId]) {
          ...
          console.log(
               `%c UNSUBSCRIBED ${subId}`,
               'background: linear-gradient(90deg, rgba(255,0,0,1) 17%, rgba(255,175,0,1) 100%); color: #ffffff; border-radius: 10px; padding: 5px'
          );
     }
};
