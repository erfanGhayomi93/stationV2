...
interface IActionColProps extends ICellRendererParams {
    handleDelete: (data: IData) => void;
    handleEdit: (data: IData) => void;
}
...
const ActionCol = ({ data, handleDelete, handleEdit }: IActionColProps) => {
    //
    const [isDeleteBtnActive, setIsDeleteBtnActive] = useState(false);

    const handleCancel = () => setIsDeleteBtnActive(false);
    // const handleDelete = () => {
    //     // dataSetter((prev) => prev.filter((item) => item.orderId !== data?.orderId))
    // };
    ...
    const handleSendRequest = () => {
        if (isObjectContainsFalsy(data, ['quantity', 'price'])) {
