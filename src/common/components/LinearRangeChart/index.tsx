    ...
    return (
        ...
        >
            ...
            originData <= thresholdData[1] ? (
                <>
                    {/* Upper numbers */}
                    <div style={{ padding: '0 9px' }} className=" lc-w-full lc-h-full">
                        <div className=" lc-relative  lc-w-full  lc-inline-flex lc-h-9  lc-items-end    ">
                            <ThresholdStart
                                data={thresholdData[0]}
                                className={thresholdSetting?.thresholdStartClassName}
                                icon={thresholdIcons?.start}
                            />
                            <Origin data={originData} thresholdData={thresholdData} {...originSetting} icon={originIcons} />
                            <ThresholdEnd data={thresholdData[1]} className={thresholdSetting?.thresholdEndClassName} icon={thresholdIcons?.end} />
                        </div>{' '}
                    </div>
                    {/*  */}
                    {/* Ruler */}
                    <div className=" lc-relative  lc-w-full  lc-inline-flex ">
                        <Ruler thresholdData={thresholdData} ruler={ruler} {...rulerSetting}>
                            <Exchange
