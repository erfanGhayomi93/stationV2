...
const ControllerInput: FC<IControllerInputType> = ({ onChange, highValue, inputValue, lowValue, children, placeholder, title, unit, max }) => {
    ...
    return (
        <label className="w-full flex items-center justify-center ">
            ...
            <div className="w-full flex border-L-gray-400 dark:border-D-gray-400 border overflow-hidden rounded-md duration-250 dark:focus-within:border-D-infoo-100 focus-within:border-L-info-100">
                <div className="w-full">
                    <TradeInput onChange={onChange} value={inputValue} placeholder={placeholder} unit={unit} max={max} />
                </div>
                <div className="flex w-48 justify-between items-center flex-row-reverse bg-L-gray-300 dark:bg-D-gray-300 border-r border-L-gray-500 dark:border-D-gray-500">
                    {children}
                    <span className="flex flex-col w-full ">
                        <button className="flex items-center justify-between px-2 gap-2" onClick={(e) => onChange(highValue)}>
                            {chevronIconUp}
                            <span className="text-L-gray-600 dark:text-D-gray-600 h-4 select-none">{seprateNumber(highValue)}</span>
                        </button>
                        ...
                        <button className="flex items-center justify-between px-2 gap-2" onClick={(e) => onChange(lowValue)}>
                            {chevronIconDown}
                            <span className="text-L-gray-600 dark:text-D-gray-600 h-4 select-none">{seprateNumber(lowValue)}</span>
                        </button>
                    </span>
                </div>
            </div>
