...
const PushEngineInfo = () => {
     ...
     const greenStatus = useMemo(
          () => ['CONNECTED:WS-STREAMING', 'CONNECTED:HTTP-STREAMING', 'CONNECTED:WS-POLLING', 'CONNECTED:HTTP-POLLING'],
          []
     );

     const signalData = useMemo(() => {
          if (redStatus.includes(pushEngineState))
     ...
     useEffect(() => {
          if (pushEngine.client)
               pushEngine.client.addListener({
     ...
     return (
          <div className={`flex ${signalData.colorClass}`}>
               <span className="mx-2 text-xs">{signalData.text}</span>
